...
model <- function(params, ord=c(), ord_name="", reps=1, name="fazly") {
  ...
  traj = matrix(0, length(ord$trials), voc_sz) # list()
  align = matrix(0, voc_sz, voc_sz) # calc from probs of stim on trial
  assoc = matrix(0, voc_sz, voc_sz) # sum of alignments over time
  probs = matrix(0, voc_sz, voc_sz) # normalized assocs
    ...
    for(t in 1:length(ord$trials)) {
      ...
      tr_probs = probs
        for(f in tr) {
          ...
          sumT = sum(tr_probs[tr,f]) + alpha*epsilon
          for(w in tr) {
            align[w,f] = (tr_probs[w,f]+epsilon) / sumT 
          }
        }
        assoc[tr,tr] = assoc[tr,tr] + align[tr,tr]
  ...
  want = list(perf=diag(probs), matrix=probs, traj=traj)
  return(want)
} 
